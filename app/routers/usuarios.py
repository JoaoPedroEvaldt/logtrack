...
from app.routers.auth import exigir_admin, get_usuario_atual
...
def atualizar_usuario(id: int, dados: UsuarioUpdate, db: Session = Depends(get_db), atual: Usuario = Depends(exigir_admin)):
    usuario = db.query(Usuario).filter(Usuario.id == id).first()
    if not usuario:
        raise HTTPException(status_code=404, detail="Usuário não encontrado")
