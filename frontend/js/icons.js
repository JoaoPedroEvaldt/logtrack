/* Conjunto de ícones em linha (estilo Feather), usados nos templates gerados por JS.
   Ícones estáticos (sidebar, cabeçalhos) ficam direto no HTML. */
const ICONES = {
  ...
  fechar_x: '<line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/>',
  ...
  reboque: '<rect x="3" y="6" width="18" height="10" rx="1"/><circle cx="7" cy="18" r="2"/><circle cx="17" cy="18" r="2"/>',
};

function svgIcone(nome, tamanho) {
  tamanho = tamanho || 16;
  const conteudo = ICONES[nome] || '';
  return `<svg width="${tamanho}" height="${tamanho}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-3px;flex-shrink:0;">${conteudo}</svg>`;
}
